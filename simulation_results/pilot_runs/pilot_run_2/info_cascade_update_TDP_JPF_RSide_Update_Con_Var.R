...
#write.csv(cascade_model_BSCM_pilot, file = "cascade_model_BSCM_pilot.csv")
# 
# loading model and adding opinion proportion to the model
cascade_model_BSCM_pilot <- data.frame(read.csv('cascade_model_BSCM_pilot_2.csv'))
...
ggplot(cascade_model_BSCM_pilot, aes(x = Links, linetype=Expertise_Influence)) +
   ...
   labs(x='Interconnectivity (%)', y='Global Proportion of Opinions', linetype='Expertise Strength') +
...
# ## Degree of Clustering
 ggplot(cascade_model_BSCM_pilot, aes(x = Links, linetype=Expertise_Influence)) +
   ...
   scale_x_continuous(breaks = round(seq(min(cascade_model_BSCM_pilot$Links) - 0.5, max(cascade_model_BSCM_pilot$Links), by = 5),1)) +  #seq(min(cascade_model_BSCM_pilot$Links), max(cascade_model_BSCM_pilot$Links), by = 2000)
   scale_y_continuous(breaks = round(seq(0, 100, by = 10),1), limits = c(0,100)) +
   theme_bw() +
   scale_color_manual(values=c("#000000")) +
   geom_vline(xintercept = .5, size = .5, colour = "black", linetype = "dashed") +
   geom_hline(yintercept = .5, size = .5, colour = "black", linetype = "dashed") +
   labs(x='Interconnectivity (%)', y='Clustering (% Like-minded Neighbors)', linetype='Expertise Strength') +
