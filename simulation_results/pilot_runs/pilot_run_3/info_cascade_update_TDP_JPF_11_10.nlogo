;; 2-D representation of a belief propagation network based on a previous model by Toby D. Pilditch (2017)
;; novel features include variations for sampling prior beliefs, a Bayesian source credibility function (Toby Pilditch, 2016) to update prior beliefs, confidence values used to influence updating behavour, and more
;;(info tab will be updated soon)
...
set prior-con sample-beta alpha_con beta_con                           ;; prior confidence drawn from beta distribution with two flexible shape parameters alpha and beta (see function sample-beta)
set prior-con min_con + (prior-con * (max_con - min_con))              ;; making sure confidence values are within the min and max boundaries


;; setting up P(H)

ifelse (prior-con > con-threshold and con-in-prior = true)[                                          ;; if we wish to include piror confidence and the value of confidence is larger than .5 (in the experiment, only those with values > 0.5 were influenced by confidence if I remember correctly)
  set p-H (stats:normal-left (random-normal .5 .2) prior-mean (prior-sd * (1 - prior-con)))]         ;; we compute the integral from 0 to x (where x is a number drawn from N(.5 .2)) for the distribtuion defined by our prior-mean and our prior-sd weighted by confidence
  [set p-H (stats:normal-left (random-normal .5 .2) prior-mean prior-sd)]                            ;; same as above excluding weighting of prior-sd by confidence


; setting up P(E) and P(E).
set p-T stats:normal-left (random-normal .5 .2) p-T-mean p-T-sd
set p-E stats:normal-left (random-normal .5 .2) p-E-mean p-E-sd

if (prior-variance = false)
    [set p-H .5
     set p-E .5
     set p-T .5]
]
...
;; ABM Function created by Toby D. Pilditch (2016) - all errors rest with me (adapted by Jan-Philipp Fränken, 2019)

; no flipping of expertise here anymore
...
;; need an overwrite if nb-prop-YN is true than you do not need to bother about the prop likelihood
;; -> may
...
;; counting neighbors congruent what has been communicated and computing their weighted mean P(E) and P(T) values
  ...
  ;; what is being said to them and wehther 0 or 1 is going to affect the integration - if communicated 0 - then BMSC should influence them to believe
  ;; so flip should be based on whether they received 0 or 1
  ;; what someone beliefs to
  ;; if red agent is sender then pick target needs to be based on red is the attempted persuasion hence in numerator
  ;; if I start at 0.3 and I've been told 0 -> I should be going down -> function can not feed 0 in -> would move to point 5 with is wrong -> actually means I need to base flipping of prior
  ;; of what was communicated to me -> should also flip prior

  ;; flip prior in the beginning if i was told 0
  ;; need to flip posterior if communicated 0
  ;; flipping independent of prior but only dependent on the message that I receive
...
"prior-con" 0.01 1 -7500403 true "" "histogram [prior-con] of turtles"
...
@#$#@#$#@
<experiments>
  <experiment name="Replication_2017" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
